...
const starsSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="#C9952A"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>';
...
function makeCard(stars, quote, name, date, source) {
    const starsHtml = Array(stars).fill(starsSvg).join('');
...
for (let colIdx = 0; colIdx < 3; colIdx++) {
    ...
    html += `            <div class="reviews-col col-${direction}">\n`;
    const cards = [];
    for (let i = 0; i < 5; i++) {
        const idx = colIdx * 5 + i;
        cards.push(makeCard(...reviews[idx]));
    }
...
fs.writeFileSync("gen_reviews.txt", html, "utf-8");
